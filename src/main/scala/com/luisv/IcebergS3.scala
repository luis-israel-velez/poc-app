...
object IcebergS3 {
    ...
    def main(args: Array[String]): Unit = {
        ...
        .config("spark.sql.catalog.spark_catalog.warehouse", "s3a://aws-prototype-s3/raw")
        .getOrCreate()
        ...
        val jsonDF = spark.read.format("json").schema(schema).load("s3a://aws-prototype-s3/contract/example.json")
        ...
        spark.sqlContext.sql(s""" CREATE OR REPLACE TABLE default.${destTable} USING iceberg AS
        ...
        spark.sqlContext.sql(s""" CREATE OR REPLACE TABLE default.pipeline_metadata USING iceberg AS
        ...
        spark.sqlContext.sql(s""" CREATE OR REPLACE TABLE default.schema_registry USING iceberg AS
